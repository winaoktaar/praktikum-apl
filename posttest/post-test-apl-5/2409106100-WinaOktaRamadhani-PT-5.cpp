...
// fungsi pointer (increment dengan dereference)
void incrementCount(int *count) {
    (*count)++;
}

// Fungsi swap dua Batik (pointer & dereference)
void swapBatik(Batik *b1, Batik *b2) {
    Batik temp = *b1;
    *b1 = *b2;
    *b2 = temp;
}

// fungsi dengan pointer parameter (cetak satu Batik)
void printBatikPointer(Batik *b) {
    cout << b->nama << " | " << b->asal << " | " << b->kategori << " | " << b->harga << endl;
}

// Deklarasi fungsi dan prosedur (pointer-based untuk registering & adding)
...
void registerUser(User *users, int *userCount) {
    ...
    // menggunakan fungsi pointer untuk increment
    incrementCount(userCount);
...
int login(User users[], int userCount, int attempts) {
    ...
    return login(users, userCount, attempts - 1);
}
...
void adminMenu(Batik batiks[], int &batikCount) {
    ...
    do {
        ...
        cout << "4. Update Batik\n";
        cout << "5. Hapus Batik\n";
        cout << "6. Swap 2 Batik (Contoh Pointer)\n";
        cout << "7. Logout\n";
        cout << "Pilih opsi: ";
        cin >> adminOption;
        cin.ignore();
        ...
        switch (adminOption) {
            ...
            case 6: {
                cout << "Masukkan indeks dua batik untuk swap (1-based): ";
                int i1, i2; cin >> i1 >> i2; cin.ignore();
                if (i1 <= batikCount && i2 <= batikCount) {
                    swapBatik(&batiks[i1-1], &batiks[i2-1]);
                    cout << "Swap berhasil!\n";
                } else {
...
void tambahBatik(Batik *batiks, int *batikCount) {
    ...
    // increment menggunakan pointer
    incrementCount(batikCount);
...
void tampilkanBatik(Batik batiks[], int batikCount) {
    ...
    for (int i = 0; i < batikCount; i++) {
        // cetak via fungsi pointer
        printBatikPointer(&batiks[i]);
    }
}
